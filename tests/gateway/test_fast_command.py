...
@pytest.mark.asyncio
async def test_run_agent_passes_structured_session_metadata_to_gateway_agent(monkeypatch, tmp_path):
    _install_fake_agent(monkeypatch)
    runner = _make_runner()
    ...
    import hermes_cli.tools_config as tools_config
    monkeypatch.setattr(tools_config, "_get_platform_tools", lambda user_config, platform_key: {"core"})
    runner._resolve_structured_session_metadata = AsyncMock(return_value=expected_metadata)
    ...
    assert result["final_response"] == "ok"
    assert _CapturingAgent.last_init["session_metadata"] == expected_metadata
